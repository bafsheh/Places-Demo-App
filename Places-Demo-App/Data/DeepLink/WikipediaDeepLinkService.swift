...
//  Places-Demo-App
//
//  Purpose: Builds Wikipedia Places URLs and opens them via the generic deep link service; adapter implements Domain port.
//  Dependencies: Location (Domain), DeepLinkServiceProtocol, OpenWikipediaAtLocationPort (Domain).
//  Usage: WikipediaDeepLinkAdapter (wrapping WikipediaDeepLinkService) is injected into OpenWikipediaUseCase; use case depends only on the port.
//
...
final class WikipediaDeepLinkAdapter: OpenWikipediaAtLocationPort {
    ...
    func openPlaces(at location: Location) async throws {
        do {
            ...
            // Map any non–DeepLinkError to domain error so the Domain boundary never leaks infrastructure errors.
            throw OpenWikipediaError.cannotOpenURL
        }
    }
}
